...
//			numberOfPlayers++;
	...
	private void remove_CPU_function() {
		...
		if (!playerContainer.getChildren().isEmpty()) {
			...
			ComboBox<String> colorComboBox = (ComboBox<String>) lastRow.getChildren().get(2);//get color Combo box from row
			ComboBox<String> tokenComboBox = (ComboBox<String>) lastRow.getChildren().get(3);//get token Combo box from row
			...
			for (Node node : playerContainer.getChildren()) {
				if (node instanceof HBox) {
					...
					ComboBox<String> rowColorComboBox = (ComboBox<String>) row.getChildren().get(2); //get color Combo box from row
					ComboBox<String> rowTokenComboBox = (ComboBox<String>) row.getChildren().get(3);//get token Combo box from row
...
//	    token.setId("token" + (index + 1)); // unique id
	 ...
	 * Retrieves player information from the playerContainer and initializes Player objects accordingly.
	 * Adds the initialized Player objects to the game data.
	 * If a player is a CPU player, initializes a cpu_Player object instead.
	 * Navigates to the game board view after initialization.
	...
	/**
	 * Redraws the specified ComboBox by removing the provided value.
	 * If the removed value was the selected item in the ComboBox, clears the selection.
	...
	/**
	 * Checks if there are duplicate player names entered in the text fields.
	 * Throws an InputIsNotUnique exception if duplicates are found.
